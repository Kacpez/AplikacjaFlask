...
api=Flask(__name__)
api.secret_key = "manbearpig_MUDMAN888"
@api.route("/", methods=['GET','POST'])
def index():
	return render_template("index.html")

@api.route("/skoczek", methods=['GET','POST'])
def skoczek():
	q1="""
...
@api.route("/skocznia/zwyciezcy", methods=['GET','POST'])
def zwyciezcy():
	q1="""
...
@api.route("/kraj/skoczkowie", methods=['GET','POST'])
def skoczkowieKraj():
	q1="""
...
@api.route("/kraj/skocznie", methods=['GET','POST'])
def skocznieKraj():
	q1="""
...
@api.route("/skocznia", methods=['GET','POST'])
def skocznia():
	q1="""
...
@api.route("/skoczek/podziwianie", methods=['GET','POST'])
def podziwianie():
	q1="""
...
@api.route("/skocznia/skoki", methods=['GET','POST'])
def skoki():
    q1="""
...
@api.route("/kraj", methods=['GET','POST'])
def kraj():
	q1="""
...
@api.route("/skocznia/statystyki", methods=['GET','POST'])
def statystyki():
	q1="""
...
@api.route("/skoczek/dodaj_skoczka", methods=["POST", "GET"])
def dodaj_skoczka():
    q1="""
...
@api.route("/skoczek/edytuj_skoczka", methods=["POST", "GET"])
def edytuj_skoczka():
    q1="""
    MATCH (p:Skoczek) RETURN p  
    """
...
@api.route("/skoczek/usun_skoczka", methods=["POST", "GET"])
def usun_skoczka():
    q1="""
    MATCH (p:Skoczek) RETURN p  
    """
...
@api.route("/skoczek/dodaj_podziwianie", methods=["POST", "GET"])
...
@api.route("/skoczek/dodaj_skok", methods=["POST", "GET"])
def dodaj_skok():
    q1="""
    MATCH (p:Skoczek) RETURN p  
    """
...
@api.route("/skoczek/dodaj_zwyciezce", methods=["POST", "GET"])
def dodaj_zwyciezce():
    q1="""
    MATCH (p:Skoczek) RETURN p  
    """
...
@api.route("/skocznia/dodaj_skocznie", methods=["POST", "GET"])
def dodaj_skocznie():
    q1="""
    MATCH (p:Kraj) RETURN p  
    """
...
@api.route("/skocznia/usun_skocznie", methods=["POST", "GET"])
def usun_skocznie():
    q1="""
    MATCH (p:Skocznia) RETURN p  
    """
...
@api.route("/kraj/dodaj_kraj", methods=["POST", "GET"])
def dodaj_kraj():
    if request.method == "POST":
...
@api.route("/kraj/usun_kraj", methods=["POST", "GET"])
def usun_kraj():
    q1="""
    MATCH (p:Kraj) RETURN p  
    """
...
@api.route("/skocznia/<usr>", methods=["POST", "GET"])
...
'''
@api.route('/dodaj_skoczka', methods=['GET','POST'])
def dodaj_skoczka():
    if request.method == 'POST':
        skoczek_nazwa = request.form['name']
        #skoczek_rok = request.form['lname']
        #skoczek_forma = request.form['email']
        query = (
            "CREATE (p1:Skoczek { nazwa: $skoczek_nazwa })"
        )
        session.run(query)
        #print(results)
        #data=results.data()
        #print(data)
        #wynik=[row["name"] for row in data]
        flash('Successfully')
        return skoczek_nazwa
    else:
        return render_template("dodaj_skoczka.html")
'''
driver.close()